...
export async function POST(req: Request) {
  try {
    const { pinCode, gate } = await req.json();
    ...
    // Multi-gate: open the gate the visitor scanned (?gate=<key> → BRIVO_DOOR_ID_<key>),
    // falling back to the single BRIVO_DOOR_ID.
    const gateKey = String(gate || '').replace(/[^A-Za-z0-9_]/g, '').slice(0, 16);
