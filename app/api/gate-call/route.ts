...
// Restrict the gate key so it maps safely to BRIVO_DOOR_ID_<gate>.
function cleanGate(v: unknown): string {
  return String(v || '').replace(/[^A-Za-z0-9_]/g, '').slice(0, 16);
}

export async function POST(request: Request) {
  try {
    const { residentPhone, residentName, visitorName, reason, gate } = await request.json();
    const gateKey = cleanGate(gate);
    ...
    if (gateKey) params.set('gate', gateKey);
    const qs = params.toString();
    const actionUrl = `${proto}://${host}/api/gate-call/keypress${qs ? `?${qs}` : ''}`;

    // Friendly gate label for the announcement (optional GATE_NAME_<gate> env).
    const property = SITE_CONFIG.propertyName || 'the';
    const gateLabel = gateKey
      ? (process.env[`GATE_NAME_${gateKey}`] || `gate ${gateKey}`)
      : 'gate';
    const announce =
      `You have a visitor at the ${property} ${gateLabel}. ` +
