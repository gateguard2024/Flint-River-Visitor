...
export async function POST(request: Request) {
  ...
  // Resolve which gate to open. Multi-gate properties tag the QR with ?gate=<key>,
  // which maps to BRIVO_DOOR_ID_<key>. Falls back to the single BRIVO_DOOR_ID.
  const gateKey = (url.searchParams.get('gate') || '').replace(/[^A-Za-z0-9_]/g, '').slice(0, 16);
  const doorId = (gateKey && process.env[`BRIVO_DOOR_ID_${gateKey}`]) || process.env.BRIVO_DOOR_ID;
  if (!doorId) {
    console.error('gate keypress: no door configured', { gateKey, tried: gateKey ? `BRIVO_DOOR_ID_${gateKey}` : 'BRIVO_DOOR_ID' });
